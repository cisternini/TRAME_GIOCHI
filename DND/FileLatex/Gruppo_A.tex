...
\paragraph{UFFICIO DEI SAGGIATORI}

\begin{itemize}
  \item Un tempo usato dai saggiatori della miniera per valutare metalli e pagare gli scavatori.
  \item La stanza appare come un vecchio ufficio o magazzino.
  \item Presente un grosso bancone di pietra con tre bilance di ferro impolverate.
  \item La parete nord ha nicchie piene di fogli di carta polverosi.
  \item Sul pavimento giacciono vecchi cadaveri (gnomi e orchi).
  \item I documenti si sbriciolano facilmente, ma chi legge il Nanico può distinguere annotazioni su pesi e rimborsi.
  \item \textbf{Tesoro:} dietro il bancone si trova una cassaforte chiusa a chiave.
  \begin{itemize}
    \item Può essere aperta con arnesi da scasso (prova di Destrezza, CD 20).
    \item Contiene: 600 monete di rame (mr), 180 monete d’argento (ma), 90 monete d’elettro (me), 60 monete d’oro (mo).
  \end{itemize}
\end{itemize}

\paragraph{ DORMITORIO A SUD}

\begin{itemize}
  \item Era il dormitorio dei minatori più esperti della Caverna dell’Onda Tonante.
  \item Origliando alla porta (prova di Saggezza, Percezione, CD 10) si sente qualcosa che si spezza e un rosicchiare costante.
  \item La stanza contiene:
  \begin{itemize}
    \item File di brandine di pietra.
    \item Un braciere arrugginito al centro con carboni spenti.
    \item Ossa di nani e orchi con frammenti di armature ancora indossati.
  \end{itemize}
  \item Tre figure grigie e ricurve (ghoul) rovistano tra i resti e rosicchiano ossa.
  \item I tre ghoul provengono dall’area 9 e sono impegnati a cercare midollo nelle ossa.
  \item I ghoul attaccano subito, desiderosi di carne fresca.
\end{itemize}

\paragraph{MAGAZZINO IN ROVINA}

\begin{itemize}
  \item La parte nord dell’area è relativamente intatta, nonostante i danni circostanti.
  \item La parete est è crollata, lasciando un cumulo di detriti.
  \item Una porta socchiusa a nord conduce a un magazzino più ampio.
  \item Lungo le pareti si trovano barilotti impilati, tutti crepati o spaccati dal tempo.
  \item Il magazzino è un luogo sicuro per riposare:
  \begin{itemize}
    \item Nessun mostro lo frequenta.
    \item La porta è in buone condizioni e può essere facilmente bloccata dall’interno.
  \end{itemize}
  \item Il contenuto dei barilotti si è evaporato da tempo.
\end{itemize}
\paragraph{8. CAVERNA DEI FUNGHI}

\begin{itemize}
  \item Nezznar ha evitato di esplorare quest’area, temendo i mostri presenti nonostante sospetti che le officine magiche siano vicine.
  \item Il terreno è ricoperto da un fitto strato di funghi di vario tipo:
  \begin{itemize}
    \item Vesce larghe 30 cm.
    \item Funghi poliporacei sulle stalagmiti.
    \item Funghi alti fino a 1,5 m.
    \item Alcune vesce brillano di fosforescenza verdastra.
  \end{itemize}
  \item La maggior parte dei funghi è innocua.
  \item Il bagliore verde consente visibilità completa senza bisogno di luce artificiale o scurovisione.
  \item \textbf{Gas Velenoso:}
  \begin{itemize}
    \item Quando una creatura attraversa la caverna, i funghi rilasciano un gas velenoso.
    \item Tiro salvezza su Costituzione, CD 11:
    \begin{itemize}
      \item Fallimento: 3d6 danni da veleno e condizione \textit{avvelenato} per 10 minuti.
    \end{itemize}
    \item Il gas si disperde dopo 1 minuto.
    \item Fino ad allora, ogni creatura che termina il turno nella caverna deve ripetere il tiro salvezza.
  \end{itemize}
\end{itemize}

\paragraph{GRANDE CAVERNA}

\begin{itemize}
  \item Un tempo utilizzata come sala banchetti, area d’incontro e sala del sidro per i minatori.
  \item La caverna è suddivisa in tre sezioni da due scarpate ripide:
  \begin{itemize}
    \item Sezione centrale più bassa.
    \item Due terrazze rialzate alle estremità.
    \item Due scalinate di pietra conducono alle terrazze.
  \end{itemize}
  \item Arredi presenti:
  \begin{itemize}
    \item Due grossi tavoli e bracieri nella sezione centrale.
    \item Un tavolo più piccolo sulla terrazza est.
  \end{itemize}
  \item Resti scheletrici di dozzine di nani, gnomi, orchi e ogre testimoniano un antico scontro violento.
  \item Sette ghoul si nascondono nella terrazza ovest:
  \begin{itemize}
    \item Attaccano non appena notano luci o rumori.
    \item Combattono fino alla distruzione, spinti dalla fame.
  \end{itemize}
  \item Le scarpate sono alte 3 metri:
  \begin{itemize}
    \item Scalata: prova di Forza (Atletica), CD 12.
    \item Caduta: 1d6 danni contundenti e condizione \textit{prona}.
  \end{itemize}
\end{itemize}
\paragraph{POZZA OSCURA}

\begin{itemize}
  \item Caverna occupata in gran parte da una pozza d’acqua ferma e scura, difficile da scrutare.
  \item Le rive sono circondate da gusci rotti di strane cozze pallide; l’aria è satura di odore di pesce.
  \item Collegamenti:
  \begin{itemize}
    \item Un passaggio a sud.
    \item Una rampa di scale verso est.
    \item Un torrente d’acqua fangosa esce a nordest, verso l’area 18.
  \end{itemize}
  \item Profondità:
  \begin{itemize}
    \item Pozza: 6 metri al centro.
    \item Torrente: 90 cm di profondità, con soffitto alto 60–90 cm.
    \item Il torrente può essere guadato facilmente.
  \end{itemize}
  \item Esplorando la pozza si trova:
  \begin{itemize}
    \item Uno scheletro umano a 3 metri dalla riva e a 3 metri di profondità.
    \item Si tratta di un mago di Phandalin, ucciso da frecce orche mentre difendeva le miniere.
  \end{itemize}
  \item \textbf{Tesoro:}
  \begin{itemize}
    \item Due anelli di platino (75 mo ciascuno).
    \item Una bacchetta dei dardi incantati.
  \end{itemize}
\end{itemize}
\paragraph{DORMITORIO A NORD}

\begin{itemize}
  \item La porta est è barricata dall’interno; per forzarla è necessaria una prova di Forza, CD 20.
  \item Origliando a una delle due porte (prova di Saggezza, Percezione, CD 10), si sentono voci rauche in Goblin che lamentano fame.
  \item La stanza è illuminata e riscaldata da un braciere fiammeggiante al centro.
  \item Lungo le pareti si trovano brandine di pietra.
  \item Se si entra da ovest, si vede:
  \begin{itemize}
    \item Una porta opposta, barricata con assi ricavate da un tavolo di legno.
  \end{itemize}
  \item Cinque bugbear occupano la stanza:
  \begin{itemize}
    \item Servono Nezznar e sorvegliano l’accesso alla sua tana (area 19).
    \item Impediscono ai non morti (ghoul, zombi, ecc.) di avvicinarsi.
  \end{itemize}
  \item La stanza era originariamente un dormitorio per minatori.
  \item I bugbear hanno rimosso i cadaveri trovati e costruito la barricata.
  \item \textbf{Tesoro:}
  \begin{itemize}
    \item Il capo bugbear ha una borsa con 15 monete di rame (mr), 13 monete d’elettro (me) e una pozione di vitalità.
  \end{itemize}
\end{itemize}
\paragraph{12. CAVERNA DELLA FONDERIA}

\begin{itemize}
  \item Nezznar non ha ancora superato questa zona per via dei guardiani non morti che bloccano l’accesso alla Forgia degli Incantesimi (area 15).
  \item La stanza ospita:
  \begin{itemize}
    \item Una fornace spenta.
    \item Un mantice meccanico alimentato da una ruota ad acqua (il canale è ora asciutto).
    \item Cumuli di carbone e metallo grezzo.
  \end{itemize}
  \item Passaggi verso ovest, sud ed est; il canale prosciugato esce a nord ed est.
  \item Più di una dozzina di cadaveri avvizziti (nani e orchi) giacciono nella stanza.
  \item Un teschio avvolto da fiamme verdi fluttua sopra di essi.
  \item Otto nani caduti si rialzano come zombi e attaccano gli intrusi:
  \begin{itemize}
    \item Inseguono solo per 1 round oltre i confini dell’area.
  \end{itemize}
  \item Il teschio infuocato è un non morto intelligente, ex servitore dei maghi alleati, che obbedisce ancora agli ordini di protezione dell’area.
  \item Questa era l’area centrale della raffinazione mineraria:
  \begin{itemize}
    \item Il torrente dell’area 18 alimentava la ruota, che azionava il mantice.
  \end{itemize}
  \item Il canale è profondo 1,5 metri; facile da attraversare.
  \item Può essere seguito verso nord o est; il soffitto del passaggio fuori dalla stanza è alto solo 1,5 metri.

\end{itemize}

\paragraph{13. CAVERNA STELLATA}

\begin{itemize}
  \item I resti scheletrici e i danni visibili testimoniano una battaglia magica devastante avvenuta secoli fa.
  \item Il soffitto è ricoperto da minerali che riflettono la luce, simulando un cielo stellato notturno.
  \item Dozzine di scheletri (alcuni schiacciati dai detriti) giacciono sul pavimento.
  \item La caverna ospita due edifici in pietra, di costruzione umana:
  \begin{itemize}
    \item Mura annerite e porte danneggiate dal fuoco.
  \end{itemize}
  \item Una scarpata divide l’area, dotata di una rampa di scale.
  \item Uscite verso nord, sud e ovest.
  \item Gli edifici vengono descritti nelle aree 14 e 15.
  \item I minerali del soffitto non sono magici e non hanno valore.
  \item Chi è competente in Arcano percepisce una lieve aura magica:
  \begin{itemize}
    \item L’aura si intensifica vicino all’edificio a nord (area 15).
  \end{itemize}
\end{itemize}
\paragraph{14. ALLOGGI DEI MAGHI}

\begin{itemize}
  \item Le porte sono crepate e i cardini parzialmente fusi:
  \begin{itemize}
    \item Per aprirle serve una prova di Forza con CD 15.
  \end{itemize}
  \item L’interno mostra segni di un’esplosione devastante:
  \begin{itemize}
    \item Pareti annerite, polvere, cenere e detriti ovunque.
    \item I mobili (tavoli, sedie, scaffali, letti) sono anneriti ma relativamente integri.
    \item Ai piedi di un letto si trova un forziere di ferro annerito.
  \end{itemize}
  \item La stanza è abitata dal wraith Mormesk:
  \begin{itemize}
    \item Non visibile inizialmente, emerge dal pavimento quando entra una creatura vivente.
    \item In vita era un potente mago umano, morto durante l’attacco degli orchi.
    \item La sua anima corrotta lo lega al tesoro contenuto nel forziere.
    \item Comanda i non morti nella Caverna dell’Onda Tonante.
  \end{itemize}
  \item La stanza era usata come alloggio per i maghi umani in visita alla Forgia degli Incantesimi (area 15).
  \item Tutti i mobili sono a misura di umano.

  \item \textbf{Interpretare Mormesk:}
  \begin{itemize}
    \item Parla con voce rauca e sussurrante.
    \item Attacca se non si tenta di comunicare o se provocato.
    \item Può essere calmato con oggetti magici, pergamene o libri arcani.
    \item È necessaria una prova di Carisma (Persuasione), CD 10, per convincerlo del valore dell’offerta.
    \item Non rinuncerà mai alla sua pipa in filigrana di platino.
    \item Può offrire monete, gemme e accesso ai suoi libri se i personaggi accettano di uccidere lo \textit{spectator} nell’area 15.
    \item Permette di consultare i suoi tomi e ottenere una mappa segreta cucita in uno di essi.
  \end{itemize}

  \item \textbf{Tesoro:}
  \begin{itemize}
    \item Il forziere contiene:
    \begin{itemize}
      \item 1.100 monete di rame (mr), 160 monete d’argento (ma), 50 monete d’elettro (me).
      \item Tre diamanti (100 mo ciascuno).
      \item Una pipa di legno con filigrana in platino (150 mo).
    \end{itemize}
    \item Sulle librerie:
    \begin{itemize}
      \item Diversi libri, alcuni di narrativa.
      \item Uno contiene una mappa segreta cucita nella copertina:
      \begin{itemize}
        \item Richiede prova di Intelligenza (Indagare), CD 12, per essere notata.
        \item Indica un dungeon scelto dal DM per una futura avventura.
      \end{itemize}
    \end{itemize}
  \end{itemize}
\end{itemize}
\paragraph{15. FORGIA DEGLI INCANTESIMI}

\begin{itemize}
  \item Centro dell'incantamento per armi naniche e congegni gnomeschi, usato dai maghi alleati del Patto di Phandelver.
  \item La porta nord è crepata e annerita; per forzarla serve una prova di Forza, CD 15.
  \item La doppia porta ovest è anch’essa danneggiata ma leggermente socchiusa.
  \item La stanza è gravemente danneggiata da un’antica battaglia magica:
  \begin{itemize}
    \item Tavoli da lavoro anneriti.
    \item Intonaco bruciato sulle pareti.
  \end{itemize}
  \item Al centro si trova un piedistallo di pietra con un braciere contenente una fiamma verdastra, intatto nonostante i danni.
  \item Sorveglia l’area uno \textbf{spectator}, creatura sferica con un grande occhio centrale e quattro occhi su tentacoli:
  \begin{itemize}
    \item Comunica telepaticamente.
    \item Era stato evocato da un mago per proteggere gli oggetti magici.
    \item È impazzito e crede che la miniera sia ancora operativa.
    \item Attacca chi tenta di rimuovere qualcosa dalla stanza.
    \item Se accecato, torna al suo piano d'origine, credendo di non poter più adempiere al suo incarico.
    \item Con una prova di Carisma (Inganno), CD 15, può essere convinto che il suo compito è concluso.
  \end{itemize}

  \item \textbf{Braciere della Fiamma Verde:}
  \begin{itemize}
    \item Identificabile con una prova di Intelligenza (Arcano), CD 15.
    \item Fonte della magia che permeava la caverna, ora in gran parte esaurita.
    \item Armi o armature immerse nella fiamma per 1 minuto diventano +1 per 1d12 ore.
    \item Il braciere non può essere rimosso.
  \end{itemize}

  \item \textbf{Stanza Nord:}
  \begin{itemize}
    \item Area separata per rifinitura degli oggetti magici.
    \item Quasi completamente distrutta.
  \end{itemize}

  \item \textbf{Tesoro:} si trova sul tavolo da lavoro a sudest:
  \begin{itemize}
    \item \textbf{Portatrice di Luce:} mazza +1 per un chierico di Lathander.
    \begin{itemize}
      \item Risplende come una torcia su comando.
      \item Infligge 1d6 danni radiosi extra ai non morti quando illuminata.
    \end{itemize}
    \item \textbf{Guardiadrago:} corazza di piastre +1 con simbolo di drago d’oro.
    \begin{itemize}
      \item Conferisce vantaggio ai tiri salvezza contro armi a soffio di creature di tipo drago.
    \end{itemize}
  \end{itemize}
\end{itemize}
