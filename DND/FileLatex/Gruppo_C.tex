...
\begin{description}
    ...
    \item[Giorno 4:] Se si sono uniti alla causa, allora arrivano direttamente al combattimento finale. Se non si sono uniti, continuano verso il Darkhold e poi Far e Jack andranno per conto loro.
        \begin{description}
            \item[Campo di battaglia:]
                    \begin{itemize}
                        \item PF Tenda Centrale : 20, PF Tende Normali : 10.
                        \item Campod battaglia : 150 metri, tutto in torno è disboscato. 
                    \end{itemize}
            \item[Truppe]:
                \begin{description}
                    \item[Nemici]: 100 Gnoll [5 gruppi da 20]
                    \item[Alleanza]: 20 satiri, 20 centuari, 15 elfi, 30 hobgoblin, 10 bugbear, 50 Goblin.    
                \end{description}
            \item[refurtiva]: 
                \begin{description}
                    \item[Nella tenda degli gnoll] : 100 mo ,2000 ma, 500 mr, Scudo sentinella[pag  198], Armatura adamantina (giacco di maglia)[pag 157], Borsa dei trucchi(marrone)[pag 168 DM], Gemma di 1000 MO. 
            ...
            \item[Tenda: ] 
              \begin{description}
                \item[Descrizione]: “Entrando nella tenda, l’odore di sangue stantio e carne bruciata vi assale come una frustata. Il suolo è coperto da fango e peli, e alle pareti pendono teschi legati con tendini. Al centro, un braciere nerofumo ribolle lentamente, mentre qualcosa di indefinito sfrigola sopra il fuoco. Sotto un mucchio di ossa, scorgete un idolo scuro dalla forma grottesca.” 
              \end{description}
        \end{description}
\end{description}
\subparagraph*{Schede Unità per Fazioni}

\begin{itemize}
  \item \textbf{Gnoll (5 Unità da 20)}
  \begin{itemize}
    \item Nome: Bracconieri Gnoll
    \item Numero: 20 per unità
    \item HP: 50
    \item AC: 13
    \item Attacco: +4, Danno: 1d8+2
    \item Morale: +1
    \item Speciale: Frenesia – +1 danno sotto il 50\% HP
  \end{itemize}

  \item \textbf{Centauri (1 Unità da 20)}
  \begin{itemize}
    \item Nome: Cavalleria Centaura
    \item Numero: 20
    \item HP: 60
    \item AC: 14
    \item Attacco: +5, Danno: 2d8 (carica)
    \item Morale: +2
    \item Speciale: Carica – +1d8 danni se in corsa
  \end{itemize}

  \item \textbf{Satiri (1 Unità da 20)}
  \begin{itemize}
    \item Nome: Schermagliatori Satiri
    \item Numero: 20
    \item HP: 40
    \item AC: 13
    \item Attacco: +3, Danno: 1d6
    \item Morale: +2
    \item Speciale: Mobilità – Ignorano terreno difficile
  \end{itemize}

  \item \textbf{Elfi Esploratori (1 Unità da 13)}
  \begin{itemize}
    \item Nome: Esploratori Elfi
    \item Numero: 13
    \item HP: 40
    \item AC: 13
    \item Attacco: +4, Danno: 2x 1d8 (a distanza)
    \item Morale: +3
    \item Speciale: Tiro da copertura – Svantaggio per chi li attacca da lontano
  \end{itemize}

  \item \textbf{Druidi Elfi (2 Indipendenti)}
  \begin{itemize}
    \item Nome: Druidi della Foresta
    \item Numero: 2
    \item HP: 30 ciascuno
    \item AC: 12
    \item Magie chiave:
    \begin{itemize}
      \item \textit{Spike Growth} – Danni da movimento
      \item \textit{Call Lightning} – AOE, 3d10
      \item \textit{Entangle} – Blocco area
    \end{itemize}
    \item Morale: N/A
    \item Speciale: Influenzano il campo intero
  \end{itemize}

  \item \textbf{Hobgoblin (1 Unità da 30)}
  \begin{itemize}
    \item Nome: Fanteria Hobgoblin
    \item Numero: 30
    \item HP: 70
    \item AC: 16
    \item Attacco: +3, Danno: 1d10
    \item Morale: +4
    \item Speciale: Formazione – +1 AC se vicini a un'altra unità
  \end{itemize}

  \item \textbf{Bugbear (1 Unità da 10)}
  \begin{itemize}
    \item Nome: Incursori Bugbear
    \item Numero: 10
    \item HP: 35
    \item AC: 14
    \item Attacco: +5, Danno: 2d6
    \item Morale: +2
    \item Speciale: Agguato – +2 attacco al primo turno
  \end{itemize}

  \item \textbf{Goblin (1 Unità da 50)}
  \begin{itemize}
    \item Nome: Marea Goblin
    \item Numero: 50
    \item HP: 50
    \item AC: 12
    \item Attacco: +2, Danno: 1d6
    \item Morale: +1
    \item Speciale: Attacco in massa – Vantaggio se oltre 30 membri
  \end{itemize}
\end{itemize}


\textbf{Svolgere la battaglia:} Ogni turno faccio un tiro fra una fazione degli Gnoll e un Fazione Alleata per capire come vanno le sorti. d20 + Modificatore necessario, se gli HP scendono sotto il 40\% tiro su morale per capire se ci possono essere svantaggi. 
...
\subsubsection{Fuori dal bosco: }
    Dopo la battaglia, separatisi da Jack e Far, come promesso vengono accompagnati da Norberto fuori dal bosco in tutta sicurezza. E qui devono prendere una decisione procedere prima verso il Darkhold o procedere verso il tempio di Lathander
    
    
    \paragraph{Montagne:}
            \subparagraph{Descrizione} Davanti a voi, a molte leghe di distanza, si alza una vasta catena montuosa che domina l’intero orizzonte occidentale. Le \textbf{Montagne del Tramonto} si presentano come un mare pietrificato di creste frastagliate e cime aguzze, punteggiate qua e là da chiazze di neve bianca che resistono anche all’estate.

Le pendici più basse sono avvolte da un fitto manto di alberi scuri --- perlopiù pini e abeti --- che sfumano in un verde profondo quasi nero. Più in alto, le foreste cedono il passo alla roccia viva: pareti scoscese, gole profonde e crinali taglienti come coltelli.

Qua e là si notano fenditure strette come cicatrici e altipiani battuti dal vento, illuminati dalla luce calda del tramonto che accende le pareti di riflessi dorati, ramati e violacei. In alcuni punti, si distinguono persino antichi sentieri o torri in rovina, appena visibili come minuscole sagome contro la vastità delle montagne.

Una sottile coltre di nubi si avvolge attorno alle vette più alte, muovendosi lenta, come se le montagne stesse respirassero. Lì sopra, il mondo sembra diverso: più antico, più silenzioso, e pieno di segreti.
    \subparagraph{Incontri casuali}
            Ogni giorno tiro un 1d20 -> 18 - 20 incontro casuale
            \begin{center}
                \begin{tabular}{|c|l|}
                \hline
                \textbf{Tiro d20} & \textbf{Incontro} \\
                \hline
                1--3   & Nani (Dwarves) \\
                4--6   & Gnoll \\
                7--9   & Ogre \\
                10--12 & Yeti \\
                13--15 & Drow (Elfi Oscuri) \\
                16--18 & Giganti del Gelo (Frost Giants) \\
                19--20 & Drago Bianco (White Dragon) \\
                \hline
                \end{tabular}
            \end{center}
  \paragraph{Verso il tempio}: 
            \subparagraph{Attraverso le montagne}
                \begin{description}
                    \item[Tempo Necessario: ]4 giorni
                    \item[Più pericoloso]: Vedere Montagne  
                \end{description}
            \subparagraph{Girare a valle:} 
                \begin{description}
                    \item[Tempo] : 8 giorni
                    \item[Meno pericolo]: Incontrano solo nani o gruppi piccoli di gnoll 
                \end{description}
            \subparagraph{Arrivo al tempio}:
                    \textbf{Descrizione Ghentilara:}
                    \begin{description}
                        \item[Origini:] Mezzelfa che unisce armoniosamente le eredità elfica e umana.

                        \item[Aspetto fisico:] Carnagione olivastra e liscia; capelli intrecciati di colore bianco argenteo; lineamenti affascinanti e occhi profondi; figura slanciata e movimenti naturalmente aggraziati.

                        \item[Abbigliamento:] Indossa abiti pratici ma eleganti, realizzati con tessuti di qualità e in colori sobri che esaltano la sua bellezza senza ostentazione.

                        \item[Carattere:] Intelligente, saggia e compassionevole; possiede equilibrio interiore e capacità di mediazione tra diversi punti di vista; dotata di empatia e presenza rassicurante.

                        \item[Ruolo nella comunità:] Figura rispettata e ammirata; guida e punto di riferimento sia nella vita quotidiana che nelle questioni politiche; simbolo di stabilità e speranza per chi la circonda.
                    \end{description}
                    \textbf{Tempio}
                    \begin{description}

  \item[Nome del tempio:] \textbf{Aurora dell’Eterna Rinascita}

  \item[Posizione:]
  Costruito su una terrazza naturale in alta quota, con vista su un’intera vallata. Il tempio è orientato verso est per catturare ogni primo raggio dell’alba, simbolo della rinascita quotidiana.

  \item[Architettura:]
  \begin{itemize}
    \item Realizzato in pietra calcarea chiara o marmo rosato che riflette dolcemente la luce solare.
    \item Grandi vetrate colorate raffigurano scene di creazione, guarigione e speranza.
    \item Colonne slanciate scolpite a forma di fiamme o boccioli in fiore.
    \item Una cupola centrale aperta verso il cielo, da cui i raggi dell’alba colpiscono direttamente l’altare.
    \item Terrazzamenti e giardini pensili adornano il tempio, con ruscelli di acqua di disgelo.
  \end{itemize}

  \item[Atmosfera:]
  \begin{itemize}
    \item L’aria è limpida e rarefatta, permeata da una sensazione di pace.
    \item All’alba, campane di cristallo suonano con armonie sacre, grazie a un antico meccanismo magico.
    \item Il rito dell’\textbf{Alba del Nuovo Giorno} viene celebrato ogni mattina da chierici e pellegrini.
  \end{itemize}

  \item[Servizi e funzioni:]
  \begin{itemize}
    \item Presenza stabile di monaci e sacerdoti di Lathander che offrono benedizioni e guarigioni.
    \item Una biblioteca sacra contenente testi su resurrezione, cicli naturali, arte e poesia.
    \item Una sala per l’accoglienza dei pellegrini, semplice ma accogliente.
    \item Una cripta chiamata \textit{Cripta del Riposo}, dove riposano eroi risorti o martiri di Lathander.
  \end{itemize}

  \item[Elementi magici:]
  \begin{itemize}
    \item Fiori incantati che sbocciano ogni mattina, anche in pieno inverno.
    \item Un altare che si illumina all’aurora, diffondendo una tenue luce dorata che ispira chi prega.
    \item Una statua di Lathander i cui occhi brillano brevemente con i primi raggi del sole.
  \end{itemize}

\end{description}
                    
\paragraph{Verso il Darkhold}
 \begin{description}

  \item[Posizione:]
  Situata sui pendii occidentali delle Montagne della Spina del Mondo (Sunset Mountains), in una gola remota e sorvegliata. Vi si accede solo attraverso un singolo e tortuoso sentiero montano.

  \item[Origini e Architettura:]
  \begin{itemize}
    \item Edificata oltre 1500 anni fa, probabilmente da giganti o titani.
    \item Le strutture principali sono imponenti, con volte alte e corridoi larghi, progettati per creature giganti.
    \item Le mura sono scure, incise con rune antiche e sembrano assorbire la luce.
  \end{itemize}

  \item[Funzione e Significato:]
  \begin{itemize}
    \item Base strategica del Cerchio Nero (Zhentarim).
    \item Centro di addestramento per mercenari, maghi oscuri, spie e assassini.
    \item Rappresenta un nodo cruciale per il commercio, la guerra e la magia nera nei Reami Occidentali.
  \end{itemize}

  \item[Occupanti:]
  \begin{itemize}
    \item Governata da un alto ufficiale degli Zhentarim, spesso un potente mago o stratega.
    \item Presidiata da cavalieri neri, mercenari, negromanti e creature evocate.
    \item Alcune voci parlano di dracolichi o demoni legati al luogo tramite antichi patti.
  \end{itemize}

  \item[Elementi Magici:]
  \begin{itemize}
    \item Il castello è intricato, pieno di illusioni permanenti, trappole arcane e passaggi segreti.
    \item Livelli sotterranei ospitano cripte, laboratori magici e reliquie oscure.
    \item Struttura semi-senziente: alcuni corridoi cambiano forma o posizione.
  \end{itemize}

  \item[Atmosfera:]
  \begin{itemize}
    \item Il cielo sopra Darkhold è spesso coperto da nubi nere, indipendentemente dal tempo esterno.
    \item Si narra che nessuna creatura buona possa riposare tranquilla tra le sue mura.
    \item Ombre senz’anima si aggirano nel cortile durante le notti senza luna.
  \end{itemize}

  \item[Utilizzo in Campagna:]
  \begin{itemize}
    \item Può servire come base nemica, roccaforte da infiltrare, o luogo dove ottenere potere proibito.
    \item Ideale per missioni di spionaggio, tradimenti, recupero di artefatti oscuri o alleanze pericolose.
  \end{itemize}
