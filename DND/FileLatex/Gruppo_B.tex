...
\paragraph{Sulle tracce di Floon}
\begin{itemize}
    \item I personaggi hanno capito che Floon è stato rapito dalla Gilda di Xanathar, scambiato per Renaer Neverember, e si trova in un nascondiglio nelle fognature.
    \item Per trovare Floon, si possono interrogare i testimoni dell'aggressione; una prova di Intelligenza (Indagare) CD 15 o 5 mo in "incentivi" permettono di seguire le tracce fino a un coperchio di metallo in un vicolo, che conduce alle fognature.
\end{itemize}
\textbf{Orientarsi nelle fognature}\\
\textit{Descrizione da leggere}: Un fetido ruscello scorre sul fondo di questo tunnel
fognario, che si dirama in due direzioni. In una delle
direzioni, notate un minuscolo simbolo disegnato con
un gessetto giallo sulla parete: un cerchio grande quanto
una mano da cui si irradiano dieci raggi equidistanti.
\begin{itemize} 
    \item Le fognature sono buie, quindi sono necessarie fonti di luce per chi non ha scurovisione.
    \item Per orientarsi nelle fognature verso il nascondiglio di Xanathar, i personaggi devono seguire dei simboli con gessetto giallo (una stilizzazione di Xanathar) posti a ogni bivio.
\end{itemize}

\textbf{Lo Scrutatore}\\
\\
\textit{Descrizione da leggere}:Dopo un'ora passata a seguire i simboli attraverso
i tunnel, giungete a un trivio dove una scala a pioli
sale all'interno di un condotto di pietra chiuso da un
coperchio di metallo circolare. Uno dei famosi simboli
è stato disegnato con il gessetto su una parete vicina e
accanto al simbolo fluttua una creatura sferica grande
quanto un pompelmo, dotata di un occhio centrale
sporgente e quattro tozzi ntacoli oculari. Quando vi
vede, vi mostra le zanne.

\begin{itemize}    
    \item Dopo un'ora di ricerca, i personaggi incontrano uno scrutatore ostile, una creatura sferica con un occhio e tentacoli, che fa da guardia a un trivio ed è in combutta con Grum'shar della Gilda di Xanathar.
    \item Sconfitto lo scrutatore, i personaggi possono proseguire seguendo il simbolo e raggiungere il nascondiglio in 5 minuti.
    \item Una scala a pioli nello stesso trivio conduce alla cantina del "Pesce Spruzzante", una taverna nel Quartiere del Porto.
\end{itemize}

\paragraph{nascondiglio della gilda di Xanathar}

\includegraphics[width=12cm, height = 10 cm]{../Mappe/NascondiglioXanathar.png}
\subparagraph{Descrizione generale}
\begin{itemize}
    \item La Gilda di Xanathar possiede diversi nascondigli sparsi nelle fognature di \textbf{Waterdeep}.
    \item La \textit{planimetria} del rifugio (mappa 1.2) può essere riutilizzata per altri nascondigli.
    \item Il capo del nascondiglio è un \textbf{mezzorco} di nome \textbf{Grum’shar}.
    \item Al momento dell’arrivo dei personaggi, Grum’shar sta \textit{interrogando Floon Blagmaar} nell’area Q7.
    \item È assistito (o osservato) da un \textbf{mind flayer} di nome \textbf{Nihiloor} (vedi Appendice B).
    \item Nessuna porta è chiusa a chiave: Grum’shar presume che solo membri della Gilda possano arrivare fin qui.
\end{itemize}

\subparagraph{Area Q1: Fulcro Centrale}
\begin{itemize}
    \item I personaggi arrivano da est, attraversando circa \textbf{30 cm di acqua e liquami fognari}.
    \item Il tunnel principale sfocia in un \textbf{fulcro circolare} con due \textit{feritoie per frecce} opposte.
    \item Due passaggi proseguono a \textbf{nord} e \textbf{sud}.
    \item Una \textbf{porta di pietra} si apre lungo un cornicione sul lato ovest.
\end{itemize}
\textbf{Descrizione da leggere:} Il tunnel fognario principale sfocia in un fulcro circolare
sulle cui pareti esterne si aprono due feritoie per frecce,
una contrapposta all'altra. Due passaggi proseguono
verso nord e sud. Una porta di pietra si apre lungo un
cornicione di pietra che corre a fianco della parete ovest. \\

\textbf{Reazioni e difese}
\begin{itemize}
    \item Se i personaggi fanno rumore o parlano a voce alta, i \textbf{goblin di sentinella} nelle aree Q2a e Q2b si svegliano.
    \item I goblin attaccano attraverso le feritoie, che forniscono loro \textbf{tre quarti di copertura}.
\end{itemize}

\textbf{Porta segreta}
\begin{itemize}
    \item È presente una \textbf{porta segreta} lungo il tunnel che conduce a sud.
    \item Può essere scoperta con una prova di \textbf{Saggezza (Percezione) CD 15}.
\end{itemize}

\subparagraph{Q2. Postazioni di Guardia}
\begin{itemize}
    \item Due \textbf{goblin} fanno la guardia, uno nell’area \textbf{Q2a} e uno in \textbf{Q2b}.
    \item Sono \textbf{alleati della Gilda di Xanathar}, provenienti da \textbf{Sottomonte}.
    \item Si affidano alla \textbf{scurovisione} per vedere, ma si sono \textbf{addormentati} durante il turno di guardia.
    \item Il loro compito sarebbe sorvegliare l’\textbf{area Q1}.
    \item I personaggi possono passare inosservati con una prova di \textbf{Destrezza (Furtività) CD 9}.
\end{itemize}

\textit{Tesoro:}
\begin{itemize}
    \item Ogni goblin possiede un borsello contenente \textbf{1d6 monete d’argento (mr)}.
\end{itemize}

\subparagraph{Q3. Stanza in Disordine}
\begin{itemize}
    \item La stanza è \textbf{piena di armi arrugginite} e \textbf{abiti di tela sparsi} sul pavimento.
    \item Funziona come \textbf{armeria} e \textbf{guardaroba} del nascondiglio.
    \item \textbf{Nessun oggetto di valore} è presente.
\end{itemize}

\subparagraph{Q4. Dormitorio Vuoto}
\begin{itemize}
    \item Contiene \textbf{sei materassi consunti}, imbottiti di paglia.
    \item \textbf{Nessun oggetto di valore} si trova nella stanza.
    \item Se gli occupanti dell’\textbf{area Q5} non sono stati eliminati, i personaggi possono \textbf{sentire movimenti} provenire da lì.
    \item Il suono è percepibile attraverso la \textbf{sottile porta di legno} che separa le due stanze.
\end{itemize}

\subparagraph{Q5. Dormitorio}
\textbf{Descrizione generale}
\begin{itemize}
    \item La stanza contiene \textbf{sei materassi consunti}, imbottiti di \textbf{paglia sporca e logora}.
    \item Vi si trova un \textbf{duergar ostile} di nome \textbf{Zemk}, membro della Gilda di Xanathar.
    \item Se \textbf{Krentz} (bandito umano, CM) è sopravvissuto alla rissa al Portale Spalancato, si trova anche lui qui.
\end{itemize}

\textbf{Situazione iniziale}
\begin{itemize}
    \item Zemk sta \textbf{barricando la porta dell’area Q6} con mobili e oggetti vari.
    \item Se presente, Krentz tenta di convincerlo a \textbf{tappare la fessura sotto la porta con coperte}.
    \item Nessuno dei due nota subito gli intrusi, a meno che i personaggi non facciano \textbf{molto rumore}.
\end{itemize}

\textbf{Comportamento e combattimento}
\begin{itemize}
    \item Se inizia un combattimento, \textbf{Zemk combatte fino alla morte}.
    \item La reazione di \textbf{Krentz} dipende dal comportamento passato dei personaggi:
    \begin{itemize}
        \item Se è stato \textbf{aiutato}, offre loro la possibilità di andarsene pacificamente.
        \item Se rifiutano, combatte al fianco di Zemk.
    \end{itemize}
    \item In caso di sconfitta di Zemk, Krentz \textbf{fugge} se può, oppure \textbf{si arrende}.
\end{itemize}

\textbf{Tesoro e subordinazione}
\begin{itemize}
    \item Né Zemk né Krentz possiedono alcun \textbf{tesoro personale}.
    \item Entrambi \textbf{rispondono a Grum’shar} (vedi area Q7).
\end{itemize}

\vspace{0.5cm}
\subparagraph{Q6. Latrina}
\textbf{Descrizione}
\begin{itemize}
    \item Nella stanza si apre un \textbf{foro sul pavimento} che conduce a una \textbf{latrina a fossa}.
    \item Una \textbf{melma grigia} è fuoriuscita dal pozzo, uccidendo due goblin inviati a eliminarla.
\end{itemize}

\textbf{Dettagli ambientali}
\begin{itemize}
    \item Le \textbf{ossa dei goblin} galleggiano nella massa melmosa della creatura.
    \item Le loro \textbf{armi corrose} giacciono poco lontano, ma risultano \textbf{inutilizzabili}.
\end{itemize}
\subparagraph{Q7. SCONTRO CON IL CAPO}
\textbf{Descrizione Mind flayer e Mezzorco} 
Una tenda consunta pende dalla parete est di una lunga
sala, al cui centro è visibile la sagoma di un mezzorco
muscoloso che indossa abiti sudici e tiene il piede sul
petto di un umano dai lunghi capelli biondo-rossastri.
Dal pugno serrato del mezzorco si sprigiona una
fiammata e la vittima geme e si agita senza poter far
niente sotto il peso della creatura.
Una figura da incubo avvolta in una veste nera se ne sta
seduta su una piattaforma rialzata sul lato sud. Gli occhi
sono grossi e lattiginosi, la pelle è violacea e gommosa e
la bocca inumana è circondata da quattro tentacoli. Tiene
in grembo e accarezza delicatamente quello che sembra
un cervello rimosso dal cranio e dotato di zampe.
\begin{itemize}
    \item La sala contiene un mezzorco (Grum'shar) che sovrasta un prigioniero (Floon), e una creatura tentacolare (Nihiloor, un mind flayer) seduta su una piattaforma con un divoracervelli.
    \item Appena vede gli avventurieri, Nihiloor tenta la fuga verso la porta ovest, aspettandosi che Grum'shar e il divoracervelli lo coprano.
    \item Nihiloor possiede un globo di pietra a forma di bulbo oculare, la chiave per il portale magico nell'area Q11 che conduce alla tana di Xanathar.
    \item Usa \textit{dominare mostri} su chiunque lo ostacoli.
    \item Grum'shar è un mago apprendista caotico malvagio (con tratti razziali da mezzorco) che attacca i personaggi per "salvarsi la faccia" dopo aver capito di aver catturato la persona sbagliata (Floon invece di Renaer).
    \item \textbf{Floon Blagmaar:} Ha 1 punto ferita. Se curato, è molto grato e cerca protezione dal gruppo.
    \item \textbf{Tesoro:} Grum'shar ha un libro degli incantesimi (camuffare se stesso, dardo stregato, mani brucianti, scudo, servitore inosservato, vita falsata). Un forziere nascosto contiene 2 pozioni di guarigione, 16 mo, 82 ma e 250 mr.
\end{itemize}

\subparagraph{Q8. PASSAGGIO DI USCITA}
\begin{itemize}
    \item Una prova di Saggezza (Percezione) con CD 10 rivela un tunnel nascosto sotto una pietra del lastricato.
    \item Il tunnel porta a nord (area Q9).
\end{itemize}

\subparagraph{Q9. CANTINA PRIVATA}
\begin{itemize}
    \item È la cantina della famiglia halfling Peabody, usata per produrre birra.
    \item Gli halfling non sanno del tunnel segreto (nascosto sotto una pietra pesante, Forza 10+ per muoverla).
    \item \textbf{Sviluppo:} Se incontrati, gli halfling si calmano facilmente. Promettono di bloccare il passaggio, ma permetteranno ai personaggi di usarlo se necessario.
\end{itemize}

\subparagraph{Q10. DORMITORI}
\begin{itemize}
    \item La stanza contiene giacigli di paglia, catene e manette arrugginite.
\end{itemize}
\subparagraph{Q11. PORTA SUL RETRO}
\textbf{Descrizione da Leggere:}L'unico elemento di questa stanza, altrimenti vuota, è
una colonna di pietra su cui è inciso un piccolo simbolo:
un cerchio perfetto con dieci raggi equidistanti che si
irradiano dalla circonferenza. AI centro del cerchio c'è
un'incisione circolare più piccola che potrebbe quasi
somigliare a un occhio senza palpebre
\begin{itemize}
    \item \textbf{Colonna Centrale:} Presenta un'incisione (cerchio perfetto, 10 raggi equidistanti dalla circonferenza, incisione centrale a forma di occhio).
    \item \textbf{Meccanismo di Apertura:} Premendo l'occhio di pietra di Nihiloor nell'incisione centrale, si apre magicamente una porta (nera, opaca, 2,4m x 1,2m) sulla parete sud.
    \item \textbf{Portale:} La porta è un portale a senso unico verso l'area X22 della tana di Xanathar (Cap. 5).
    \item \textbf{Chiusura:} Il portale si chiude 1 minuto dopo la rimozione dell'occhio di pietra.
    \item \textbf{Porta Poco Segreta:} Una porta segreta è chiaramente visibile a chi si avvicina da est.
\end{itemize}

\subparagraph{Q12. CANTINA DELL'OSTELLO}
Uno stretto passaggio (le creature Medie devono stringersi) conduce alla cantina di un ostello halfling (Via delle Spezie, Quartiere del Porto).
\begin{itemize}
    \item \textbf{Occupanti:} L'ostello è la base degli Schivaframmenti, una banda di halfling topi mannari.
    \item \textbf{Alleanza:} Mantengono un'alleanza precaria con la Gilda di Xanathar e conoscono il loro nascondiglio.
    \item \textbf{Guardia:} Roscoe Underbough (topo mannaro halfling, forma ibrida) è di guardia nel magazzino a nordovest.
    \item \textbf{Comportamento di Roscoe:} Cerca di spaventare gli intrusi ma preferisce non uccidere, consentendo la ritirata.
    \item \textbf{Statistiche Roscoe (Modifiche):}
    \begin{itemize}
        \item \textbf{Taglia:} Piccola.
        \item \textbf{Punti Ferita:} 27 (6d6 + 6).
        \item \textbf{Tratti Razziali:} Può muoversi attraverso lo spazio di creature Medie o superiori; Vantaggio ai TS per non essere spaventato.
        \item \textbf{Lingue:} Comune, Halfling, Gergo Ladresco.
    \end{itemize}
    \item \textbf{Sviluppo:} Se i personaggi superano Roscoe, trovano l'ostello frequentato da halfling. Gli altri Schivaframmenti sono assenti e il personale accoglie i PG, indirizzandoli al bagno pubblico.
\end{itemize}


\paragraph{Ricercati} 
\subparagraph{Al e Vik }:
    I due PG insieme a Penelope sono ricercati per omicidio del locandiere del Drago Infilzato, purtroppo hanno ignorato il piccolo Tim, che è scappato dopo averli visti, il fato vuole che ha incontrato due guardie, a cui in maniera un raffazzonata ha dato una descrizione dei 3. Le guardie, dopo un giorno, hanno diramato delle locandine con dei volti molto simili ai 3, con scritto "Per ordine dei Lord è stata emanata una taglia di 200 mo per questa persona: vivo o morto" a testa.
    Se si mettono in cammino la notte potrebbero riuscire a uscire dalla città, ma se decidono di restare, dovranno fare attenzione a non farsi riconoscere.
    Se restano in città, mentre camminano vedono delle guardie che si fermano davanti ad un bacheca e appendono dei fogli. Se si avvicinano notano le locandine. 
    \textbf{TIri fortuna per decidere se ci sono guardie in giro:}
    \begin{itemize} 
        \item 1-5: Non ci sono guardie in giro.
        \item 6-15: Ci sono 1d4 guardie in giro, ma non sembrano interessate ai PG.
        \item 16-20: Ci sono 1d4 guardie in giro che sembrano cercare qualcuno che corrisponde alla descrizione dei PG. 
        \item Modificatori quartieri:
        \begin{itemize}
            \item Quartiere del Porto: +2 alle probabilità di incontrare le guardie.
            \item Quartiere dei Mercanti: +1 alle probabilità di incontrare le guardie.
            \item Porte: +3 alle probabilità di incontrare le guardie.
            \item Altri quartieri: nessun modificatore.
    \end{itemize}
    \end{itemize}
    \textbf{Eventi possibili}
    \begin{itemize}
        \item Se i PG vengono fermati dalle guardie, possono tentare di corromperle (CD 15 Carisma Intimidire o Persuasione) o mentire (CD 15 Carisma Inganno) per evitare l'arresto. In caso di fallimento, verranno arrestati e portati in prigione.
        \item Se i PG vengono fermati, ma scappano, inizia una scena di inseguimento.
        \item Se i PG evitano le guardie, possono tentare di rimuovere le locandine (CD 10+1*ogni guardia nei ditornti Destrezza Furtività) per ridurre le probabilità di essere riconosciuti.
        \item Se i PG decidono di lasciare la città, devono superare una prova di Destrezza Furtività CD 15 (Vantaggio se sono travestiti) per evitare di essere notati dalle guardie alle porte della città.
    \end{itemize}

    \textbf{Se vengono arrestati:}
    \begin{itemize}
        \item I PG vengono portati in prigione e processati. Possono tentare di corrompere il giudice (CD 19 Carisma Intimidire o Persuasione) o mentire (CD 15 Carisma Inganno) per evitare la condanna. In caso di fallimento, verranno condannati a 1d10 anni di prigione. 
        \item Se riescono a corrompere il giudice, la condanna viene ridotta a 1d4 anni di prigione.
\end{itemize}    
    \subparagraph{Penelope}
    Se riesce a portare a termine la sua missione, dovrà per forza incotrare anche le guardie Staget, che un pò riluttante sarà costretto a fermarla per interrogarla. Penelope potrà tentare di corrompere le guardie (CD 15 Carisma Intimidire o Persuasione) o mentire (CD 15 Carisma Inganno) per evitare l'arresto o convincere della sua innocenza nell'omicidio del locandiere. Potrebbe avere il supporto di Reaner e Volo, e avere uno sconto di pena. 
\begin{itemize}
    \item \textbf{Aggredire un cittadino:} Incarcerazione fino a una decade, fustigazione e risarcimento fino a 1.000 mo.
    \item \textbf{Danneggiamento di proprietà o bestiame:} Risarcimento a coprire il costo delle riparazioni o della sostituzione più una sanzione massima di 500 mo.
    \item \textbf{Disturbo alla quiete pubblica:} Multa fino a 25 mo e editto.
    \item \textbf{Furto con scasso:} Incarcerazione fino a 3 mesi e risarcimento pari al valore delle merci rubate più 500 mo.
    \item \textbf{Omicidio di un cittadino per futili motivi:} Morte o lavori forzati fino a 10 anni e risarcimento fino a 1.000 mo da pagare ai parenti della vittima.
    \item \textbf{Omicidio di un cittadino per gravi motivi:} Esilio fino a 5 anni o lavori forzati fino a 3 anni o risarcimento fino a 1.000 mo da pagare ai parenti della vittima.
    \item \textbf{Rapina:} Lavori forzati fino a 1 mese e risarcimento pari al valore delle merci rubate più 500 mo.
    \item \textbf{Ricatto o intimidazione di un cittadino:} Multa o risarcimento fino a 500 mo e editto.
    \item \textbf{Schiavismo:} Fustigazione e lavori forzati fino a 10 anni.
    \item \textbf{Uso della magia per influenzare un cittadino senza il suo consenso:} Multa o risarcimento fino a 1.000 mo e editto.
\end{itemize}
